...
store['ORE'] = {spare: 1000000000000, used: 0};
...
let count = 0;
while (true) {
  (count % 10000 === 0) && console.log('MAKING FUEL', ++count, 'HAVING ORE', store['ORE'].spare);
  try {
    produce(formulas, store, fuelEF);
  } catch (e) {
    console.log('NO MORE ORE');
    console.log(store);
    console.log('ANSWER 14-2', store['FUEL']); // 2390226
    break;
  }
  // console.log(store);
}
...
function produce(formulas, store, eF) {
  ...
  eF.s.forEach(requirements => {
    ...
    if (componentName === 'ORE') {
      if (store[componentName].spare >= requiredAmount) {
