...
function main(instructions) {
  let src = instructions.split("\n")
    .map(route => route.split(","));
  // console.log(instructions, src);
  ...
  pos = "0,0";
  src[0].forEach((move,i) => {
    const subRoute = getAllCoords(pos, move);
    // console.log("0: SubRoute", i, subRoute);
    addSubRoute(route, subRoute);
    pos = subRoute[subRoute.length-1];
    // if (i%20 === 0) console.log(i);
  });

  pos = "0,0";
  src[1].forEach((move,i) => {
    const subRoute = getAllCoords(pos, move);
    addSubRoute(route, subRoute, intersections, false);
    pos = subRoute[subRoute.length-1];
    // if (i%20 === 0) console.log(i);
  });
  ...
  const is = Array.from(intersections)
    .map(e => e.split(',').map(i => +i))
    .map(e => [Math.abs(e[0]), Math.abs(e[1])])
    .map(e => e[0] + e[1])
    .reduce((acc, dist) => dist < acc ? dist : acc);

  console.log("Part1:", is);

  const distances = Array.from(intersections)
    .reduce((acc, e) => {
      acc[e] = [];
      return acc;
    }, {});
  // console.log("Distances:", distances, 'from', intersections);

  src.forEach(seq => {
    let pos = ["0,0", 0];
    seq.forEach((move,i) => {
      pos = getDistances(pos, move, intersections, distances);
    });
  });

  console.log("Distances:", distances);

  const sums = Object.entries(distances)
    .map(e => e[1][0] + e[1][1])
    .reduce((acc, e) => e < acc ? e : acc);


  console.log("Part 2:", sums);
}

function addSubRoute(r, sr, is = false, add = true) {
  sr.forEach(loc => {
    if (is && r.has(loc)) {
     is.add(loc);
    }
...
function getAllCoords(start, move) {
  start = start.split(",").map(e => +e);
  const dir = move[0];
  const distance = +move.slice(1);
  const factor = (dir === 'L' || dir === 'D') ? -1 : 1;
  const orientations = {'L': 'h', R: 'h', 'U': 'v', 'D': 'v'};
  const orientation = orientations[dir];
  const stops = [];

  for (let i = 1; i <= distance; i++) {
    if (orientation === 'h') {
      stops.push([start[0] + i * factor, start[1]].join(","));
    } else {
      stops.push([start[0], start[1] + i * factor].join(","));
    }
  }
  // console.log("getAllCoords", stops);
  return stops;
}

function getDistances(start, move, is, distances) {
  // console.log(start);
  let wentSoFar = start[1];
  start = start[0].split(",").map(e => +e);
  const dir = move[0];
  const distance = +move.slice(1);
  const factor = (dir === 'L' || dir === 'D') ? -1 : 1;
  const orientations = {'L': 'h', R: 'h', 'U': 'v', 'D': 'v'};
  ...
  for (let i = 1; i <= distance; i++) {
    wentSoFar++;
    nextStop = (orientation === 'h')
      ? [start[0] + i * factor, start[1]].join(",")
      : [start[0], start[1] + i * factor].join(",");
    if (is.has(nextStop)) {
      distances[nextStop].push(wentSoFar);
    }
  }
  return [nextStop, wentSoFar];
}
