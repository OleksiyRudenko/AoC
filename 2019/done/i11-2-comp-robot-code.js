...
while (true) {
  if (vm.state === 'halted') break;
  const panel = panels.find(e => e.c[0] === robot.c[0] && e.c[1] === robot.c[1]);
  const color = panel ? panel.color : 0;
  // console.log('>>> PANELS');
  // console.log('ROBOT', robot, 'COLOR', color, 'PANEL', panel);
  const cmd = vm.run(color);
  // console.log(cmd);
  if (!cmd.length) break;
  if (panel) {
    panel.color = cmd[0];
  } else {
    // console.log('NEW PANEL', cmd[0], robot.c);
    panels.push({c: [...robot.c], color: cmd[0]});
  }
  robot.dir += cmd[1] ? 1 : -1;
  if (robot.dir === -1) robot.dir = 3;
  if (robot.dir === 4) robot.dir = 0;
  switch (robot.dir) {
    case 0: robot.c[1]--; break;
    case 1: robot.c[0]++; break;
    case 2: robot.c[1]++; break;
    case 3: robot.c[0]--; break;
  }
}

console.log(panels.length, panels);

const br = panels.reduce((br, e) => {
  if (e.c[0] > br[0]) br[0] = e.c[0];
  if (e.c[1] > br[1]) br[1] = e.c[1];
  return br;
}, [0, 0]);
...
for (let i = 0; i < br[1]; i++) {
  const row = new Array(br[0]).fill('#');
  canvas.push(row);
}

panels.forEach(e => {
  canvas[e.c[1]][e.c[0]] = e.color ? '#' : '.';
});

console.log('ANSWER 11-2');
canvas.forEach(row => {
