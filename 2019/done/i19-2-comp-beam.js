...
console.log("ANSWER 19-2", lastAnswer);
...
function main(input) {
  ...
  const offsX = 360, offsY = 600; // detected manually
  for (let j = 0 + offsY; j < offsY + 500; j++) {
    let row = '';
    let rowRes = [0, 0];
    for (let i = 0 + offsX; i < offsX + 500; i++) {
      const vm = new VM("R", input, []);
      const r = +vm.run([i, j]);
      const v = r > 0 ? r : 0;
      if (rowRes[0] === 0 && v > 0) {
        rowRes[0] = i;
    ...
    //console.log(padNumber(j, 4),
      // row,
      // rowRes, rowRes[1] - rowRes[0]);
    // console.log(padNumber(i, 4), rowRes);
    if (rowC >= 99) {
      if (rowsV[rowC-99][1] - rowRes[0] + 1 === 100) {
        console.log(rowsV[rowC-99], rowRes);
        const coords = [rowsV[rowC-99][1] - 100 + 1, rowC-99 + offsY];
        console.log(coords);
        return coords[0] * 10000 + coords[1];
      }
    }
    rowC++;
  }
