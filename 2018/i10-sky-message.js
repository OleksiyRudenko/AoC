let stars = `position=< 41710,  52012> velocity=<-4, -5>
...
position=< 31320,  52012> velocity=<-3, -5>`.split("\n")
  .map(s => s.match(/(-?[0-9]+)/g).map(n => +n));

[skyW, skyH] = draw(stars);
[nextSkyW, nextSkyH] = [skyW, skyH];
let count = 0;
while (nextSkyW <= skyW && nextSkyH <= skyH) {
  if (count++ % 100 === 0) console.log([skyW, skyH], [nextSkyW, nextSkyH]);
  [skyW, skyH] = [nextSkyW, nextSkyH];
  [nextSkyW, nextSkyH] = move(stars);
}
move(stars, -1);
draw(stars, true);
console.log(count);

function move(stars, progress = 1) {
  stars.forEach((star, idx) => {
    stars[idx][0] += star[2] * progress;
    stars[idx][1] += star[3] * progress;
  });
  return draw(stars, false);
}

function draw(stars, draw = false) {
  [minX, maxX, minY, maxY] = stars.reduce((acc, star) => {
    if (star[0] < acc[0]) acc[0] = star[0];
    if (star[0] > acc[1]) acc[1] = star[0];
    if (star[1] < acc[2]) acc[2] = star[1];
    if (star[1] > acc[3]) acc[3] = star[1];
    return acc;
  }, [stars[0][0], stars[0][1], stars[0][0], stars[0][1]]);
  const w = maxX - minX + 1,
    h = maxY - minY + 1;
  ...
  if (draw) {
    console.log(w, h);
    const sky = new Array(h).fill(null, 0, h)
      .map(() => new Array(w).fill(' ', 0, w));
    stars.forEach(star => { sky[star[1] - minY][star[0] - minX] = '*'; });
    sky.forEach(row => {
